...
export class DeviceService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/devices");
  ...
  update(device: IDevice): Observable<EntityResponseType> {
    return this.http.put<IDevice>(
      `${this.resourceUrl}/${getDeviceIdentifier(device) as number}`,
      device,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(device: IDevice): Observable<EntityResponseType> {
    return this.http.patch<IDevice>(
      `${this.resourceUrl}/${getDeviceIdentifier(device) as number}`,
      device,
      { observe: "response" }
    );
  ...
  addDeviceToCollectionIfMissing(
    deviceCollection: IDevice[],
    ...devicesToCheck: (IDevice | null | undefined)[]
  ): IDevice[] {
    const devices: IDevice[] = devicesToCheck.filter(isPresent);
    if (devices.length > 0) {
      const deviceCollectionIdentifiers = deviceCollection.map(
        (deviceItem) => getDeviceIdentifier(deviceItem)!
      );
      const devicesToAdd = devices.filter((deviceItem) => {
        const deviceIdentifier = getDeviceIdentifier(deviceItem);
        if (
          deviceIdentifier == null ||
          deviceCollectionIdentifiers.includes(deviceIdentifier)
        ) {
          return false;
        }
