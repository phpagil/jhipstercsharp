...
export class NotificationService {
  ...
  create(notification: INotification): Observable<EntityResponseType> {
    return this.http.post<INotification>(this.resourceUrl, notification, {
      observe: "response",
    });
  }
  ...
  update(notification: INotification): Observable<EntityResponseType> {
    return this.http.put<INotification>(
      `${this.resourceUrl}/${
        getNotificationIdentifier(notification) as number
      }`,
      notification,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<INotification>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  addNotificationToCollectionIfMissing(
    ...
    const notifications: INotification[] =
      notificationsToCheck.filter(isPresent);
    if (notifications.length > 0) {
      ...
      const notificationsToAdd = notifications.filter((notificationItem) => {
        const notificationIdentifier =
          getNotificationIdentifier(notificationItem);
        if (
          notificationIdentifier == null ||
          notificationCollectionIdentifiers.includes(notificationIdentifier)
        ) {
          return false;
        }
